	...
	def Insert(self,
		...
		if len(self._Items) == 0:
			...
			if ExistingName == None:
				raise ValueError(
					"Tree.Insert ERROR: I don't know where to place this new optical element. You gave me a name I can not find")
				return 0
...
class OpticalElement(TreeItem):
   ...
   #	- If Name is None, then the name is automatically assigned.
	...
	def __init__ (self,
		...
		OpticalElement._LastCreated = 100
		OpticalElement.A = 120
	...
	# ================================================
	@classmethod
	def SetLastCreated(cls,NewObject):
		cls._LastCreated = NewObject
		pass
	# ================================================
	#     __str__
	# ================================================
...
#					oeX = oeY.Parent  # even if oeY.Parent = None, that's ok. The function will handle it.
	...
	def ApplyPositioningDirectives(oeY: OpticalElement):
		...
		if oeY.IsSource == False:
			oeX = oeY.Parent if oeY.Parent != None else 'None'  # Get XY coordinates from the oeX
			oeXSameOrientation = oeY.GetParent(SameOrientation=True)
