...
#		return rm.ComputeSampling(Lambda, z, L0, L1, Alpha0, Alpha1, oe1.ComputationSettings.OversamplingFactor)
   ...
   #==========================================
	...
	#================================================
	#	FUN: GetParent [OpticalElement]
	#================================================
	def GetParent(self, SameOrientation = False):
		'''
		Returns the parent of the self OpticalElement

		If SameOrientation == True, then returns the first upstream OpticalElement with the same orientation.
		'''
...
#		Tree.__init__(self)
	...
	#================================================
	def RefreshPositions___TBDONE(self):
		'''
		DevNotes
		-----
		Modified 20191121
...
#		oeStart = oeList[0]
		...
		k = 0
		Ind = 0
		NOpticalElements = len(self.ItemList)
		for i, oeThis in enumerate(oeList):
			StrCounter = '[%i/%i]' % (i+1, NOpticalElements)
			Debug.Print('\n\n' + 80 * '=',0)
			Debug.Print( StrCounter + ' Propagate Fields [oeLast]-->>-[oeThis]----------',0)
			Debug.Print(80 * '=',0)
			oeLastStr = oeThis.Parent.Name if oeThis.Parent is not None else "None"
			Debug.Print('oeLast: ' +  oeLastStr + '\t(parent)' ,1 )
			Debug.Print('oeThis: ' +  oeThis.Name  + '\t(now processing)' ,1 )
			Debug.Print('oeThis.Ignor: ' +  str(oeThis.ComputationSettings.Ignore),1 )
			#----------------------------------------------
			...
			#----------------------------------------------
				...
				if PropInfo.oeLast.CoreOptics._IsAnalytic == True:
					Action = 'oeLast is Analytical. Evaluating analytical function (of previous OE on THIS one)'
					Debug.print('Action: ' + Action,Ind+1)
					...
					Debug.print(oeV.Name + '.UseFigureError = ' + str(oeV.CoreOptics.ComputationSettings.UseFigureError),Ind+1)
...
#		PD.GrazingAngle = GrazingAngle
	...
	def ApplyPositioningDirectives(oeY: OpticalElement):
		...
		We need two References:
			one for measuring the distance: oeXDist
			one for effectively lay out the new OpticalElement: oeXLay.

			oeXLay shall alway be the Last OpticalElement with the same orientation.
			oeXDist can be selected according to the positioning directives.

			With respect to previous code:
				oeX -> oeXDist
				oeXSameOrientation -> oeXLay
		oeX
		'''

		#XXX Working here 1
		# Get the previous item and the closest previous item with the same orientation
		if oeY.IsSource == False:
			oeXDist = oeY.Parent if oeY.Parent is not  None else None  # Get XY coordinates from the oeX
			oeXLay = oeY.GetParent(SameOrientation = True)
		...
		#======================================================================
			...
			RayIn = oeXLay.CoreOptics.RayOutNominal  # The incident ray
			LastXY = oeXSameOrientation.CoreOptics.XYCentre # XY position of the same orientation

			#==================================================================
			#  put oeY at distance "Distance" from oeX.centre
			#=================================================================
			if Pd.What == 'centre' and Pd.Where == 'centre':

				# Correction of the distance (if needed)
				if Pd.ReferTo == 'source':
					DistanceToUse = Pd.Distance - oeX.DistanceFromSource
				else:
					DistanceToUse = Pd.Distance
					#TODO we should throw an error if the new Distance is <0
				...
				if ('detector' in oeY.Name) or ('detector' in oeX.Name):
					newXYCentre = LastXY + DistanceToUse * tl.Normalize(RayIn.v)
...
#					MeasureDistance()
					# Measure the distance between the LastOpticalElementSameOrientation and oeY
					#TODO perhaps define a function MeasureDistance(... SamerOrientation = True)
			...
			# ........................................
			#  put "upstream focus" into centre of oeX
			# ........................................
			elif Pd.What == 'upstream focus' and Pd.Where == 'centre':
				#TODO: FOCUS Positioning! Dangerous check here
				# Set position
				if Pd.ReferTo == 'source':
#					oeSource = oeX # #FIXED? used to be correct in the older version...
					oeSource = oeY.Source # this should be the right version now

					while oeSource.IsSource != True:
						oeSource = oeSource.Parent

					#CAVEAT Which RayIn, of the source or the previous element?
					...
					oeY.CoreOptics.SetXYAngle_UpstreamFocus(oeXSameOrientation.XYCentre, RayIn.Angle)
